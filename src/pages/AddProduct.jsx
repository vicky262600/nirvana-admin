  ...
  const addVariant = () => {
    if (newVariant.size && newVariant.color && newVariant.quantity) {
      ...
      if (!variantExists) {
        setFormData(prev => ({
          ...prev,
          variants: [...prev.variants, { ...newVariant, quantity: parseInt(newVariant.quantity) }]
        }));
    ...
    <AddProductContainer>
      <FormWrapper>
        ...
        <Form onSubmit={handleSubmit}>
                ...
                <CategoryInput>
                  {formData.categories.map((category, index) => (
                    <CategoryTag key={index}>
                      {category}
                      <RemoveCategory onClick={() => removeCategory(category)}>×</RemoveCategory>
                    </CategoryTag>
                  ))}
                </CategoryInput>
              )}
                ...
                <VariantsList>
                  {formData.variants.map((variant, index) => (
                    <VariantItem key={index}>
                      ...
                      <RemoveVariant onClick={() => removeVariant(index)}>×</RemoveVariant>
                    </VariantItem>
                  ))}
                </VariantsList>
              )}
          ...
          <SubmitButton type="submit" disabled={!formData.title || !formData.description || formData.images.length === 0 || formData.variants.length === 0}>
            Create Product
          </SubmitButton>
        </Form>
      </FormWrapper>
